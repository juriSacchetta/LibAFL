...
use libafl_qemu::{
    cmplog::CmpLogObserver, edges::{edges_map_mut_slice, MAX_EDGES_NUM}, fibers::{input::QemuFibersInput, mutator::QemuFibersSeedMutator}, helper::QemuHelperTuple, Emulator, QemuExecutor, QemuHooks
};
...
pub type ClientState =
    StdState<QemuFibersInput<BytesInput>, InMemoryOnDiskCorpus<QemuFibersInput<BytesInput>>, StdRand, OnDiskCorpus<QemuFibersInput<BytesInput>>>;
...
impl<'a, M: Monitor> Instance<'a, M> {
        ...
        QT: QemuHelperTuple<ClientState> + Debug,
    {
        ...
        let mut harness = |input: &QemuFibersInput<BytesInput>| harness.run(input);
        ...
        if self.options.is_cmplog_core(self.core_id) {
            ...
            let interleaving_stage = StdMutationalStage::new(QemuFibersSeedMutator::new());


            // The order of the stages matter!
            let mut stages = tuple_list!(calibration, tracing, i2s, power, interleaving_stage);
            ...
            let mut stages = tuple_list!(StdMutationalStage::new(mutator), interleaving_stage);
