...
use libafl_qemu::{
    ...
    elf::EasyElf,
    fibers::{input::QemuFibersInput, QemuFibersSchedulerHelper},
...
pub type ClientState = StdState<QemuFibersInput<BytesInput>,
    InMemoryOnDiskCorpus<QemuFibersInput<BytesInput>>,
    StdRand,
    OnDiskCorpus<QemuFibersInput<BytesInput>>>;
...
impl<'a> Client<'a> {
    ...
    pub fn run<M: Monitor>(
        ...
        let extra_tokens: Option<Vec<String>> = None;
        ...
        if is_asan && is_cmplog {
            ...
            instance.build().run(
                tuple_list!(edge_coverage_helper, QemuCmpLogHelper::default(),QemuFibersSchedulerHelper::default()),
                state,
            )
        } else {
            instance
                .build()
                .run(tuple_list!(edge_coverage_helper, QemuFibersSchedulerHelper::default()), state)
        }
    }
}
