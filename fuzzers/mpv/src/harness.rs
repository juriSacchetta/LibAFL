...
use libafl_qemu::{fibers::input::QemuFibersInput, ArchExtras, CallingConvention, Emulator, GuestAddr, GuestReg, MmapPerms, Regs};
...
impl<'a> Harness<'a> {
    ...
    pub fn run(&self, input: &QemuFibersInput<BytesInput>) -> ExitKind {
        self.reset(&input.get_input()).unwrap();
        ExitKind::Ok
    }
