...
use crate::{client::ClientState, harness::Harness, options::FuzzerOptions};
...
impl<'a, M: Monitor> Instance<'a, M> {
        ...
        QT: QemuHelperTuple<ClientState> + Debug,
    {
        ...
        let mut harness = |input: &QemuFibersInput<BytesInput>| harness.run(input.get_input_ref());
        ...
        if self.options.is_cmplog_core(self.core_id) {
            ...
            let seed_mutational_stage = StdMutationalStage::new(QemuFibersSeedMutator::default());

            // The order of the stages matter!
            let mut stages = tuple_list!(calibration, tracing, i2s, power, seed_mutational_stage);
